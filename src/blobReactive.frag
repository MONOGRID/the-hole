...
uniform sampler2D iChannel0; // needs to be a data texture
...
vec2 map(in vec3 p) 
{
	...
	for(int i=0; i<NUMFREQ; i++) {
    	...
    	freqs[i]=texture2D(iChannel0,vec2(t,0.25)).x;
        b=sin(PI*t+iGlobalTime*1.3);
        c=t*a;
        d=sdSphere(p+vec3(cos(c*0.9+iGlobalTime*2.)*b,sin(c+iGlobalTime*1.7)*b,2.0*t-1.0)*0.1,0.+freqs[i]*pow(1.1,float(i))*0.09);        
        total+=d;
        minD=smin(minD,d,0.04);
        //if (d<minD) { minD=d; }
...
#define RM_MAX_STEPS 30.0
#define RM_EPSILON 0.002
#define RM_MIN_STEP_SIZE 0.00
#define RM_MIN_DISTANCE 0.0
...
)
{
    ...
    for (float i = 0.0; i < RM_MAX_STEPS; i+=1.0) 
    {    
        ...
        if (travel_distance > RM_MIN_DISTANCE && 		// total ray length is over minimum
            distance_to_scene < RM_EPSILON) 	// distance to obj is below epsilon
        {	
            return vec3(travel_distance,i/RM_MAX_STEPS,object_id); // return distance
    	}
...
void main(  )
{
    ...
    if (result.x!=0.0) {
		...
		vec3 col = vec3(specular*diffuse)*vec3(1.,1.,1.)+vec3(diffuse*0.4+0.6);
        // vec3 col = vec3(specular*diffuse) * vec3(1.0,1.0,1.0) + vec3(diffuse*0.4+0.6) * vec3(0.9,0.5 - result.y * 0.5,(1.0 / result.x)*0.1)

        color=vec4(col,1.0);
                                                                             
                                                                             
        vec4 env = textureCube(iChannel1, -normal);
        color = mix(color, env,0.2);
    }   
